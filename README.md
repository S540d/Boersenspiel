...
| `src/boersenspiel/dashboard.py` | Renders simulation results as `docs/index.html`, one `docs/<slug>.html` detail page per strategy, `docs/vergleich.html` (the full comparison table), `docs/portfolio.html` (buckets and instruments) and `docs/praemissen.html` (premises and assumptions). A switch in the three-dot menu ([#91](https://github.com/S540d/Boersenspiel/issues/91)) puts every figure on every page onto one common, much longer evaluation period — the history from the replacement bond's first price, under the assumption from [#80](https://github.com/S540d/Boersenspiel/issues/80) — instead of each strategy's own window; both sets of numbers are computed at build time, the browser only swaps them, and the switch is deliberately not remembered across reloads. Since [#88](https://github.com/S540d/Boersenspiel/issues/88) the start page carries only an introduction (why, and the basic two-bucket idea), the key learnings, the CAGR bar chart and the value-history charts; the comparison table, the CAGR-vs-drawdown scatter chart and the instrument list live on their own pages, reachable from the three-dot menu. Every value-history chart on the start and detail pages also gets a client-side observation-period switcher (1/3/5 years back, full history, or — where it actually extends the history — a fifth "extended" preset that assumes capital not yet allocatable to a target instrument sat in a single common bond ETF, IBCL, instead of cropping the period, [#80](https://github.com/S540d/Boersenspiel/issues/80)), backed by fully re-simulated presets per strategy computed at build time ([#54](https://github.com/S540d/Boersenspiel/issues/54)). The overview table's lead column re-simulates every strategy over a **common comparison period** and reports each one's excess return over the benchmark ([#73](https://github.com/S540d/Boersenspiel/issues/73)), and has an info-tooltip on every metric column header ([#81](https://github.com/S540d/Boersenspiel/issues/81)). Every fetched instrument is listed with its full name ([#79](https://github.com/S540d/Boersenspiel/issues/79)) on the portfolio page, the CAGR-vs-max-drawdown scatter chart ([#82](https://github.com/S540d/Boersenspiel/issues/82)) sits next to the comparison table |
| `src/boersenspiel/templates/` | Jinja templates: `base.html.j2` (shared shell incl. the three-dot menu), `dashboard.html.j2`, `strategy_detail.html.j2`, `vergleich.html.j2`, `portfolio.html.j2`, `praemissen.html.j2` |
